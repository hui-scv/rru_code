 ...
 * 版本描述：v1.0
 * 编写日期：2020.1.20
 * 文件描述：cpri1接口的具体事务处理文件，cpri接口是作为客户端来使用。
 */
 ...
 * 功能描述：cpri1接口线程处理函数，向BBU获取接口的ip地址，并与BBU建立TCP/IP链接，
...
/*
 * 函数名：int cpri1_handle(char *msg, int acq, int *num)
...
/*
 * 函数名：int cpri1_creatsk(int type)
...
/*
 * 函数名：int cpri1tobbu_req(RRU_HEAD *cpri_que, BBU_HEAD *cpri_ans, struct sockaddr_in *cpri_addr)
 * 功能描述：cpri1将不断地向BBU端口发送UDP广播，直到获取RRU的ip地址。
 * 		   获取到ip地址后，cpri1将自己的ip地址设置为此地址。
 * input：
...
/*
 * 函数名：int cpri1_tcpcon(BBU_HEAD cpri_ans, struct sockaddr_in *cpri_addr)
 * 功能描述：cpri1将创建使用TCP协议的套接字，然后与BBU端口进行TCP连接。
 * input：
