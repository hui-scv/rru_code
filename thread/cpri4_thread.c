 ...
 * 版本描述：v1.0
 * 编写日期：2020.1.20
 * 文件描述：cpri4接口的具体事务处理文件，cpri接口是作为客户端来使用。
 */
...
//定义告警标志位
unsigned int cpri4_ala_flag = 0;
char cpri4_bbu_ip[20];

int cpri4_creatsk(int type);
int cpri4_handle(char *msg, int acq, int *num);
int cpri4tobbu_req(RRU_HEAD *cpri_que, BBU_HEAD *cpri_ans, struct sockaddr_in *cpri_addr);
int cpri4_tcpcon(BBU_HEAD cpri_ans, struct sockaddr_in *cpri_addr);

CPRI_STATUS_S cpri4_status;

/*
 * 函数名：void *cpri4_thread(void)
 * 功能描述：cpri4接口线程处理函数，向BBU获取接口的ip地址，并与BBU建立TCP/IP链接，
...
void *cpri4_thread(void)
{
	...
	//将各种结构体清零
	memset(&cpri4_addr, 0, sizeof(struct sockaddr_in));
	memset(&cpri4_que, 0, sizeof(RRU_HEAD));
	memset(&cpri4_ans, 0, sizeof(BBU_HEAD));
	memset(&msg_head, 0, sizeof(MSG_HEAD));
	memset(msg, 0, sizeof(char) * 512);

	//指定socket套接字所采用的协议簇是IPv4
	cpri4_addr.sin_family = AF_INET;
	//指定服务器端口号为33333
	cpri4_addr.sin_port = htons(SERVER_PORT);
...
CHLINK:
	...
	*****************/
	cpri4_status.state = 1;
	do{
		//cpri_status_read(0, 0, &cpri_status_1);
	}while((cpri4_status.state & 0x01) != 1);
	
	//RRU向BBU发送UDP广播，获取IP地址并设置
	cpri4tobbu_req(&cpri4_que, &cpri4_ans, &cpri4_addr);

	//RRU向BBU发起TCP链接请求并完成链接
	sk = cpri4_tcpcon(cpri4_ans, &cpri4_addr);

	//RRU向BBU发起通信通道链路建立请求,完成RRU通信通道的配置以及返回配置响应
	cpri4_comch_init(sk, msg, &msg_head, cpri4_ans);
...
/*
 * 函数名：int cpri4_handle(char *msg, int acq, int *num)
...
int cpri4_handle(char *msg, int acq, int *num)
{
	...
	switch(msg_id)
	{
		case CPRI_CHLINK_CFG:					//通道连接配置
			ret = cpri4_comch_cfg(acq, msg);
			break;
		case CPRI_VERDOWN_QUE:					//版本下载请求
			ret = cpri4_verdown_que(acq, msg);
			break;
		case CPRI_VERACT_IND:					//版本激活指示
			ret = cpri4_veract_ind(acq, msg);
			break;
		case CPRI_STATE_QUE:					//状态查询请求
			ret = cpri4_state_que(acq, msg);
			break;
		case CPRI_PARA_QUE:						//参数查询请求
			ret = cpri4_para_que(acq, msg);
			break;
		case CPRI_PARACFG_QUE:					//参数配置请求
			ret = cpri4_paracfg_que(acq, msg);
			break;
		case CPRI_DELAYMSE_QUE | CPRI_DELAYCFG_CMD:		//时延测试请求和时延配置命令
			ret = cpri4_delaymse_que(acq, msg);
			break;
		case CPRI_ALA_QUE:						//告警上报请求
			ret = cpri4_ala_que(acq, msg);
			break;
		case CPRI_LOGUP_QUE:					//日志上传请求
			ret = cpri4_logup_que(acq, msg);
			break;
		case CPRI_RESET_IND:					//复位指示
			ret = cpri4_reset_ind(acq, msg);
			break;
		case CPRI_BBUBEAT_MSG:					//BBU心跳包消息
			ret = cpri4_bbubeat_msg(acq, msg, num);
			break;
		case CPRI_LTE_CFG:						//LTE小区配置
			ret = cpri4_lte_cfg(acq, msg);
			break;
		default:
			printf("msg_id error!\n");
			return -2;
...
/*
 * 函数名：int cpri4_creatsk(int type)
...
int cpri4_creatsk(int type)
{
	char *device = ETH3, ip[20];
	int sk = 0;
	struct sockaddr_in cpri4_client_addr;
	struct ifreq ifreq;

	memset(&cpri4_client_addr, 0, sizeof(struct sockaddr_in));
	...
	do
	{
		...
		if(sk < 0)
		{
			printf("cpri4 socket error!\n");
			continue;
		}
		printf("cpri4 socket success!\n");
		...
		printf("ip4:%s\n", ip);
		cpri4_client_addr.sin_family = AF_INET;
		cpri4_client_addr.sin_port = htons(33334);
		cpri4_client_addr.sin_addr.s_addr = inet_addr(ip);

		//最后使用此网口的ip地址绑定套接字
		if(bind(sk, (struct sockaddr*)&cpri4_client_addr, sizeof(struct sockaddr)) < 0)
		{
			printf("cpri4 bind error!\n");
			close(sk);
			sk = -1;
			sleep(3);
			continue;
		}
		printf("cpri4 bind success!\n");
...
/*
 * 函数名：int cpri4tobbu_req(RRU_HEAD *cpri_que, BBU_HEAD *cpri_ans, struct sockaddr_in *cpri_addr)
 * 功能描述：cpri4将不断地向BBU端口发送UDP广播，直到获取RRU的ip地址。
 * 		   获取到ip地址后，cpri4将自己的ip地址设置为此地址。
 * input：
...
int cpri4tobbu_req(RRU_HEAD *cpri_que, BBU_HEAD *cpri_ans, struct sockaddr_in *cpri_addr)
{
	...
	*****************/
	...
	sk = cpri4_creatsk(SOCK_DGRAM);

	do
	{
		//获取cpri4接口使用的网口的mac地址
...
/*
 * 函数名：int cpri4_tcpcon(BBU_HEAD cpri_ans, struct sockaddr_in *cpri_addr)
 * 功能描述：cpri4将创建使用TCP协议的套接字，然后与BBU端口进行TCP连接。
 * input：
...
int cpri4_tcpcon(BBU_HEAD cpri_ans, struct sockaddr_in *cpri_addr)
{
	...
	sprintf(cpri4_bbu_ip, "%d.%d.%d.%d", cpri_ans.bbu_ip[0], cpri_ans.bbu_ip[1], cpri_ans.bbu_ip[2], cpri_ans.bbu_ip[3]);
	(cpri_addr->sin_addr).s_addr = inet_addr(cpri4_bbu_ip);

	//创建使用TCP协议的套接字sk
	sk = cpri4_creatsk(SOCK_STREAM);
	...
	do
	{
		...
		{
			printf("cpri4 connect error!\n");
		}
		sleep(3);
	}while(ret < 0);

	printf("cpri4 connect success!\n");
	//返回成功连接的套接字sk
	return sk;
}
