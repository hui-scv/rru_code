 ...
 * 版本描述：v1.0
 * 编写日期：2020.1.20
 * 文件描述：cpri7接口的具体事务处理文件，cpri接口是作为客户端来使用。
 */
...
//定义告警标志位
unsigned int cpri7_ala_flag = 0;
char cpri7_bbu_ip[20];
...
CPRI_STATUS_S cpri7_status;

/*
 * 函数名：void *cpri7_thread(void)
 * 功能描述：cpri7接口线程处理函数，向BBU获取接口的ip地址，并与BBU建立TCP/IP链接，
...
CHLINK:
	...
	*****************/
	cpri7_status.state = 1;
	do{
		//cpri_status_read(0, 0, &cpri_status_1);
	}while((cpri7_status.state & 0x01) != 1);
...
/*
 * 函数名：int cpri7_handle(char *msg, int acq, int *num)
...
/*
 * 函数名：int cpri7_creatsk(int type)
...
/*
 * 函数名：int cpri7tobbu_req(RRU_HEAD *cpri_que, BBU_HEAD *cpri_ans, struct sockaddr_in *cpri_addr)
 * 功能描述：cpri7将不断地向BBU端口发送UDP广播，直到获取RRU的ip地址。
 * 		   获取到ip地址后，cpri7将自己的ip地址设置为此地址。
 * input：
...
/*
 * 函数名：int cpri7_tcpcon(BBU_HEAD cpri_ans, struct sockaddr_in *cpri_addr)
 * 功能描述：cpri7将创建使用TCP协议的套接字，然后与BBU端口进行TCP连接。
 * input：
...
int cpri7_tcpcon(BBU_HEAD cpri_ans, struct sockaddr_in *cpri_addr)
{
	...
	sprintf(cpri7_bbu_ip, "%d.%d.%d.%d", cpri_ans.bbu_ip[0], cpri_ans.bbu_ip[1], cpri_ans.bbu_ip[2], cpri_ans.bbu_ip[3]);
	(cpri_addr->sin_addr).s_addr = inet_addr(cpri7_bbu_ip);
