...
extern unsigned int cpri2_ala_flag;
extern char cpri2_bbu_ip[20];

/*
 * 函数名：int cpri2_comch_req(int sk, BBU_HEAD cpri_ans)
...
/*
 * 函数名：int cpri2_comch_cfg(int sk, char *msg)
...
4);
				irmodeans[1].res = irmodecfg[1].ir_mode;		//这里将设置状态查询中ir口工作模式查询的内容
				irmodecfgans[1].res = 0;		//设置参数配置成功
...
/*
 * 函数名：void cpri2_comch_init(int sk, char *msg, MSG_HEAD *msg_head, BBU_HEAD cpri_ans)
 * 功能描述：cpri2将不断向BBU端口发送通信链路建立请求消息，直到BBU正确应答。
 * input：
 * 		1、sk，具有TCP连接的套接字；
		...
		if(rec_num >= ((MSG_HEAD *)msg)->msg_size && ((MSG_HEAD *)msg)->msg_size != 0)
		{
			...
			if(((MSG_HEAD *)msg)->msg_id == 2)
			{
				...
				if(ret >= 0)
				{
					...
					msg_head->msg_size = MSG_HEADSIZE + chlinkans[1].ie_size;	//设置消息体的总大小
					chlinkans[1].res = 0;		//设置通道建立配置应答的结果为成功
					memcpy(send_msg, (char *)msg_head, MSG_HEADSIZE);
			...
			chlinkans[1].res = 1;		//设置通道建立配置应答的结果为失败
			memcpy(send_msg, (char *)msg_head, MSG_HEADSIZE);
			memcpy(send_msg + MSG_HEADSIZE, (char *)(&chlinkans[1]), sizeof(CL_CHLINKANS));
			send(sk, send_msg, msg_head->msg_size, 0);
...
/*
 * 函数名：int cpri2_verdown_que(int sk, char *msg)
 * 功能描述：RRU版本下载请求处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
...
int cpri2_verdown_que(int sk, char *msg)
{
	...
	{
		memcpy((char *)(&softchk[1]) + 4, (char *)src_addr, sizeof(CL_SOFTCHK) - 4);
		if(softchk[1].res == 0)		//判断接收到的版本下载请求ie，看软件版本是否一致
		{
			verdownans[1].soft_type = softchk[1].soft_type;
			verdownans[1].res = 1;		//拒绝版本下载，在这里给出拒绝原因，因为软件版本一致
		...
		}else
		{
			...
			//进行ftp下载文件
			ret = ftp_down(ETH1, cpri2_bbu_ip, softchk[1].file_name, softchk[1].file_path, 1);
			verdownres[1].soft_type = softchk[1].soft_type;
			if(ret == -5)
				verdownres[1].res = 1;		//给出版本下载的结果
			else
				verdownres[1].res = 0;
...
/*
 * 函数名：int cpri2_veract_ind(int sk, char *msg)
 * 功能描述：RRU版本激活指示处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
int cpri2_veract_ind(int sk, char *msg)
{
	...
	if(ie_id == 6 && count == size)
	{
		...
		***********************************/
		...
		msg_head.msg_size = MSG_HEADSIZE + 2 * rruveractans[1].ie_size;
		memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
		rruveractans[1].soft_type = 0;	//设置激活应答的类型是软件
		rruveractans[1].res = flag[1];		//设置软件激活的结果
		memcpy(send_msg + MSG_HEADSIZE, (char *)(&rruveractans[1]), sizeof(VA_RRUVERACTANS));
		rruveractans[1].soft_type = 1;	//设置激活应答的类型是固件
		rruveractans[1].res = flag[1];		//设置固件激活的结果
		memcpy(send_msg + MSG_HEADSIZE + rruveractans[1].ie_size, (char *)(&rruveractans[1]), sizeof(VA_RRUVERACTANS));
...
/*
 * 函数名：int cpri2_state_que(int sk, char *msg)
 * 功能描述：RRU状态查询处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
int cpri2_state_que(int sk, char *msg)
{
	...
	for(i = 0; i < 8; i++)
	{
		...
		{
			case 302:		//射频通道状态查询
				memcpy((char *)(&rfchsta[1]) + 4, (char *)src_addr, sizeof(SQ_RFCHSTA) - 4);	//取得查询请求的消息体内容
				msg_head.msg_size = MSG_HEADSIZE + rfchans[1].ie_size;
				rfchans[1].ch_num = rfchsta[1].ch_num;		//设置要查询的射频通道号
				rfchans[1].ul_sta += 0;		//设置上行通道状态响应，这里不需要设置，因为在参数配置时会进行设置它。
				rfchans[1].dl_sta += 0;		//设置下行通道状态响应，这里不需要设置，因为在参数配置时会进行设置它。
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
			...
			case 303:		//载波状态查询
				...
				cirans[1].ant_num += 0;		//设置天线组号，这里不需要设置，因为在参数配置时会进行设置它。
				cirans[1].cir_num += 0;		//设置载波号，这里不需要设置，因为在参数配置时会进行设置它。
				cirans[1].sta += 0;		//设置载波使能或非使能状态，这里不需要设置，因为在参数配置时会进行设置它。
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
			...
			case 306:		//RRU运行状态查询
				...
				runans[1].sta += 0;		//设置RRU运行状态，这里不需要设置，因为在通道建立配置和小区配置时会进行设置它。
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
			...
			case 307:		//Ir口工作模式查询
				...
				irmodeans[1].res += 0;	//设置Ir口的工作模式，这里不需要设置，因为在参数配置时已经对它进行了设置。
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
...
/*
 * 函数名：void cpri2_get_systime(CL_SYSTIME *cl_systime)
 * 功能描述：系统时间获取函数。
 * input：
 * 		1、cl_systime，系统时间结构体的指针。
 * output：void
 */
void cpri2_get_systime(CL_SYSTIME *cl_systime)
{
	time_t t;
	struct tm *area;
...
/*
 * 函数名：int cpri2_para_que(int sk, char *msg)
 * 功能描述：RRU参数查询处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
int cpri2_para_que(int sk, char *msg)
{
	...
	for(i = 0; i < 9; i++)
	{
		...
		{
			case 401:		//系统时间查询
				cpri2_get_systime(&systime[1]);	//获取本地系统时间到系统时间查询响应结构体
			...
			case 402:		//CPU占用率查询
				...
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);		//此功能已实现，开了一个定时器信号，每隔一段时间就运算出CPU占用率，并填写到cpurateans[1]结构体中
			...
			case 403:		//CPU占用率统计周期查询
				...
				ratecycans[1].rate_cyc += 0;		//设置CPU占用率统计周期，这里不需要设置，因为在参数配置时已经对它进行了设置。
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
			...
			case 404:		//RRU温度查询
				...
				rrutemans[1].tem_type = 0;		//设置温度查询类型为射频通道温度，因为只有这一个选项
				...
				break;
			case 405:		//驻波比状态查询
				memcpy((char *)(&swrsta[1]) + 4, (char *)src_addr, sizeof(PQ_SWRSTA) - 4);	//取得查询请求的消息体内容
				msg_head.msg_size = MSG_HEADSIZE + swrstaans[1].ie_size;
				swrstaans[1].rfch_num = swrsta[1].rfch_num;
			...
			case 406:		//驻波比门限查询
				...
				swrthrans[1].no1_thr += 0;		//设置驻波比一级门限，这里不需要设置，因为在参数配置时已经对它进行了设置。
				swrthrans[1].no2_thr += 0;		//设置驻波比二级门限，这里不需要设置，因为在参数配置时已经对它进行了设置。
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
			...
			case 407:		//过温门限查询
				...
				temthrans[1].tem_max += 0;		//设置过温上门限，这里不需要设置，因为在参数配置时已经对它进行了设置。
				temthrans[1].tem_low += 0;		//设置过温下门限，这里不需要设置，因为在参数配置时已经对它进行了设置。
				memcpy(send_msg, (char *)&msg_head, MSG_HEADSIZE);
				memcpy(send_msg + MSG_HEADSIZE, (char *)(&temthrans[1]), sizeof(PQ_TEMTHRANS));
				send(sk, send_msg, msg_head.msg_size, 0);	//过温门限查询响应消息体
				break;
			case 408:		//输出功率查询
				memcpy((char *)(&outpower[1]) + 4, (char *)src_addr, sizeof(PQ_OUTPOWER) - 4);	//取得查询请求的消息体内容
				msg_head.msg_size = MSG_HEADSIZE + outpowerans[1].ie_size;
				outpowerans[1].rfch_num = outpower[1].rfch_num;
...
/*
 * 函数名：int cpri2_set_systime(PC_SYSTIME *cl_systime)
 * 功能描述：系统时间设置函数。
 * input：
 * 		1、cl_systime，系统时间结构体的指针。
 * output：
 ...
 * 		失败：1
 */
int cpri2_set_systime(PC_SYSTIME *cl_systime)
{
	struct timeval tv;
	struct tm area;
	int ret;
...
/*
 * 函数名：int cpri2_paracfg_que(int sk, char *msg)
 * 功能描述：RRU参数配置处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
int cpri2_paracfg_que(int sk, char *msg)
{
	...
	for(i = 0; i < 8; i++)
	{
		...
		{
			case 11:		//参数配置-系统时间
				memcpy((char *)(&systimecfg[1]) + 4, (char *)src_addr, sizeof(PC_SYSTIME) - 4);	//取得参数配置的消息体内容
				systimecfgans[1].res = cpri2_set_systime(&systimecfg[1]);		//调用set_systime函数来配置系统时间，成功返回0，失败返回1
			...
			case 501:		//参数配置-IQ数据通道配置
				memcpy((char *)(&iqdatachcfg[1]) + 4, (char *)src_addr, sizeof(PC_IQDATACH) - 4);	//取得参数配置的消息体内容
				/************************************
			...
			case 502:		//参数配置-CPU占用率统计周期配置
				memcpy((char *)(&ratecyccfg[1]) + 4, (char *)src_addr, sizeof(PC_RATECYC) - 4);	//取得参数配置的消息体内容
				ratecycans[1].rate_cyc = ratecyccfg[1].rate_cyc;		//这里将配置参数查询中CPU占用率统计周期的查询内容
				//CPU占用率统计周期配置，采用ratecycans[1].rate_cyc这个值去设置。
				new_value.it_interval.tv_sec = ratecycans[1].rate_cyc;
				new_value.it_interval.tv_usec = 0;
				setitimer(ITIMER_REAL, &new_value, NULL);		//正式设置统计周期，没隔此时间端便计算一次
				ratecyccfgans[1].res = 0;		//设置参数配置成功
			...
			case 503:		//参数配置-驻波比门限配置
				memcpy((char *)(&swrthrcfg[1]) + 4, (char *)src_addr, sizeof(PC_SWRTHR) - 4);	//取得参数配置的消息体内容
				swrthrans[1].no1_thr = swrthrcfg[1].no1_thr;		//这里将配置参数查询中驻波比门限的查询内容
				swrthrans[1].no2_thr = swrthrcfg[1].no2_thr;		//这里将配置参数查询中驻波比门限的查询内容
				/************************************
				驻波比门限配置，这里也不知道具体怎么配，但是最后就是采用swrthrans[1]结构体中的一级二级门限值来配置
				************************************/
				swrthrcfgans[1].res = 0;		//设置参数配置成功
			...
			case 504:		//参数配置-Ir口工作模式配置
				memcpy((char *)(&irmodecfg[1]) + 4, (char *)src_addr, sizeof(CL_IRMODECFG) - 4);	//取得参数配置的消息体内容
				irmodeans[1].res = irmodecfg[1].ir_mode;		//这里将设置状态查询中ir口工作模式查询的内容
				/************************************
			...
			case 505:		//参数配置-过温门限配置
				memcpy((char *)(&temthrcfg[1]) + 4, (char *)src_addr, sizeof(PC_TEMTHR) - 4);	//取得参数配置的消息体内容
				temthrans[1].tem_max = temthrcfg[1].tem_max;		//这里将设置参数查询中过温门限查询的内容
				temthrans[1].tem_low = temthrcfg[1].tem_low;		//这里将设置参数查询中过温门限查询的内容
				/************************************
				过温门限配置，这里也不知道怎么配置
				************************************/
				temthrcfgans[1].tem_type = temthrcfg[1].tem_type;
				temthrcfgans[1].res = 0;		//设置参数配置成功
			...
			case 302:		//参数配置-射频通道状态配置
				memcpy((char *)(&rfchstacfg[1]) + 4, (char *)src_addr, sizeof(PC_RFCHSTA) - 4);	//取得参数配置的消息体内容
				/************************************
				...
				************************************/
				...
				break;
			case 507:		//参数配置-天线配置
				memcpy((char *)(&antcfg[1]) + 4, (char *)src_addr, sizeof(PC_ANTCFG) - 4);	//取得参数配置的消息体内容
				/************************************
				天线配置，这里也不知道怎么去配
				************************************/
				antcfgans[1].ant_num = antcfg[1].ant_num;
				antcfgans[1].res = 0;		//设置参数配置成功
...
/*
 * 函数名：int cpri2_delaymse_que(int sk, char *msg)
 * 功能描述：RRU时延测量处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
/*
 * 函数名：void cpri2_get_ala(AR_ALAQUE *ala_que, AR_ALAREP *ala_rep)
 ...
 * output：void
 */
void cpri2_get_ala(AR_ALAQUE *ala_que, AR_ALAREP *ala_rep)
{
	int i = 0;
	time_t t;
	struct tm *area;
	...
	for(i = 0; i < 23; i++)
	{
		...
		}else
		{
			...
			if(ala_que->ala_code == *(unsigned int *)((char *)(&alaexl[1]) + i*4))
			{
				ala_rep->ala_able = 0;		//告警码存在

				//告警码是否已经被置位
				if((cpri2_ala_flag & (1 << i)) != 0)
				{
...
/*
 * 函数名：int cpri2_ala_que(int sk, char *msg)
 * 功能描述：RRU报警查询处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
int cpri2_ala_que(int sk, char *msg)
{
	...
	{
		memcpy((char *)(&alaque[1]) + 4, (char *)src_addr, sizeof(AR_ALAQUE) - 4);	//取得参数配置的消息体内容
		cpri2_get_ala(&alaque[1], &alarep[1]);		//查询告警号是否存在，并且判断其是否被置位
...
/*
 * 函数名：int cpri2_logup_que(int sk, char *msg)
 * 功能描述：RRU日志上传请求处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
int cpri2_logup_que(int sk, char *msg)
{
	...
	{
		memcpy((char *)(&upque[1]) + 4, (char *)src_addr, sizeof(LOG_UPQUE) - 4);	//取得参数配置的消息体内容
		/************************************
...
/*
 * 函数名：int cpri2_reset_ind(int sk, char *msg)
 * 功能描述：RRU复位指示处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
/*
 * 函数名：int cpri2_bbubeat_msg(int sk, char *msg, int *num)
 * 功能描述：RRU在位检测处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
...
/*
 * 函数名：int cpri2_lte_cfg(int sk, char *msg)
 * 功能描述：RRU小区配置处理函数。
 * input：
 * 		1、sk，具有TCP连接的套接字；
 * 		2、msg，接收到的BBU端口回复的信息；
